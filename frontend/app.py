...
def load_research_data():
    ...
    if os.path.exists(demand_path) and os.path.exists(solar_path):
        ...
        if existing_apps:
            ...
            # Use the clip to ensure we don't have negative grid pull
...
if df is not None:
    # =========================================================
    # 1. GLOBAL DASHBOARD (TOP OF HOME SCREEN)
    # =========================================================
    ...
    # Global Calculations for the 24-hour horizon
    total_load_val = df['total_demand'].sum()
    optimized_load_val = df['optimized_load'].sum() # This will display the ~29.15 kWh result
    ...
    with col1:
        ...
        st.caption("Baseline energy demand.")
    ...
    with col2:
        ...
        st.caption("Remaining grid dependency.")

    with col3:
        st.metric(label="Total Cost Optimization", value=f"${total_savings:.2f}", 
                  delta=f"{(total_savings/baseline_cost*100):.1f}% Savings")
        st.caption("Financial savings via PPO scheduling.")
    ...
    with xai_col:
        ...
        if row['solar_gen'] > 4.0:
            msg = f"Decision for hour {selected_hour} is Solar-Driven ({row['solar_gen']:.2f} kW)."
        else:
            msg = f"Decision for hour {selected_hour} is Grid-Driven (${grid_prices[selected_hour]:.2f}/kWh)."
        st.info(msg)
    ...
    st.error("🚨 System Offline: Data files missing in /data folder.")
