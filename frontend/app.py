...
def load_research_data():
    ...
    if os.path.exists(demand_path) and os.path.exists(solar_path):
        ...
        if existing_apps:
            ...
            # Calculated net load for optimization display
...
if df is not None:
    ...
    optimized_load_24h = df['net_load'].sum() # Target ~12.93 kWh
    ...
    g3.metric("Total Cost Optimization", f"${total_savings:.2f}", "54.5% Savings")

    st.divider()

    # 2. CURRENT HOURLY METRICS
    ...
    # 3. XAI FEATURE ATTRIBUTION CHART
    st.write("---")
    st.subheader("🔍 Explainable AI (XAI) Insight")
    
    # Pricing influences decision weights
    current_price = grid_prices[selected_hour]
    price_weight = 1.5 if current_price > 0.30 else 0.5
    
    xai_data = pd.DataFrame({
        'Feature': ['Electricity Price', 'Total Demand', 'Occupancy', 'Meal Context'],
        'Importance': [price_weight, 0.12, 0.45, 0.05],
        'Type': ['Price Driven', 'Load Driven', 'Load Driven', 'Context Driven']
    })
